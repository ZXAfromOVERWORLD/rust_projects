...
fn load_tasks() -> Vec<Task> {
    let file = OpenOptions::new()
        ...
        .create(true)
        .open("tasks.json")
        .expect("Error of SOME DIPSHIT KIND");
...
fn save_tasks(tasks: Vec<Task>) {
    let file = OpenOptions::new()
        ...
        .truncate(true)
        .open("tasks.json")
        .expect("Error While Writing this code");
...
fn modify_task(tasks: &mut Vec<Task>) {
    let idx = get_input("Which task to change? ID:".to_string())
        .trim()
        .parse::<i32>()
        .unwrap();
    for i in tasks {
...
fn get_input(prompt: String) -> String {
    ...
    io::stdin()
        .read_line(&mut ans)
        .expect("Error Please Enter a valid input");
